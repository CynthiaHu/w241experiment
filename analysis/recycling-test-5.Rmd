...
```{r}
# more plots to explore covariates

# checks the different splits of our population
# splits on our houses
dtA[,.(.N),by=c("Pre")]
dtA[Pre=="N",.(.N),by=c("Treat")]

# splits on our streets
dtA[,.(.N),by=c("Pre","Street")][,.(.N),by=Pre]
dtA[Pre=="N",.(.N),by=c("Treat","Street")][,.(.N),by=Treat]
...
ggplot(data=dtA[S==1,.(street.score,neighbors.score,Treat)], mapping=aes(x=street.score,y=neighbors.score,color=Treat)) +
    geom_point(size=2) +
    ggtitle("Street score vs Neighbor score")
# no clear pattern between street score and neighbor score
```
